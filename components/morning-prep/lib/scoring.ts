...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { rhythm: 0, info: 0, flex: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { rhythm: 0, info: 0, flex: 0 };
...
/**
 * 나와 궁합이 맞는 유형(=준비 속도는 서로 보완하면서 아침을 대하는 결은 맞는
 * 상대)의 축 조합. `rhythm`(준비 리듬) 극만 반대로 뒤집고, `info`(정보 습관)와
 * `flex`(컨디션 대응)는 나와 같은 유형을 찾는다.
 *
 * 근거: 준비 리듬이 같은 사람끼리는(둘 다 여유형이면 서로 늦장을 눈감아주기
 * 쉽고, 둘 다 벼락치기형이면 둘 다 정신없어지는 것처럼) 서로의 속도를
 * 보완해주기 어렵다. 리듬이 반대면 한쪽이 여유 있게 챙기고 다른 쪽이 속도를
 * 내는 자연스러운 균형이 생긴다. 반면 아침에 정보를 확인하는 습관(info)과
 * 컨디션에 대응하는 방식(flex)까지 다르면 같은 아침을 대하는 결이 어긋나기
 * 쉬워, 이 두 축은 나와 같은 쪽을 찾는다.
 *
...
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    rhythm: poles.rhythm === "high" ? "low" : "high",
  };
}

...
const POLE_PHRASES: Record<string, { high: string; low: string }> = {
  rhythm: { high: "여유 있게 준비하는 편", low: "시간에 쫓기듯 몰아서 준비하는 편" },
  info: { high: "아침부터 알림과 소식을 확인하는 편", low: "알림보다 내 루틴에 먼저 집중하는 편" },
  flex: { high: "정해둔 순서를 그대로 지키는 편", low: "그날그날 상황에 맞게 순서를 바꾸는 편" },
};
...
/**
 * 궁합 유형은 준비 리듬만 반대이고 정보 습관·컨디션 대응은 나와 같은 유형이다.
 * (src/lib/scoring.ts의 compatiblePoles 참고)
 */
function compatibilityReasonText(outcome: Outcome): string {
  const infoLabel = POLE_LABELS.info[outcome.axes.info.pole];
  const flexLabel = POLE_LABELS.flex[outcome.axes.flex.pole];
  return `준비 리듬은 나와 반대라서 서로 속도를 보완해주고, 정보 습관(${infoLabel})과 컨디션 대응(${flexLabel})은 나와 같아서 아침을 대하는 결이 잘 맞아요.`;
}
  ...
  return (
    <main className="screen result-screen">
      ...
      <div ref={captureRef} className="result-capture">
        <p className="eyebrow">나의 아침 준비 유형은</p>
        ...
        <p className="capture-note">재미로 보는 출근 준비 스타일 테스트 · 나는 아침에 어떤 사람?</p>
      </div>
      ...
      <section className="panel">
        <h2 className="panel-title">내일 아침에 해보면 좋아요</h2>
      ...
      <p className="limit-note">
        이번 응답에서는 이 유형과 가장 가까웠어요. 아침의 모습은 전날 컨디션과
        그날 일정에 따라 달라질 수 있고, 이 결과는 재미로 즐기는
        엔터테인먼트예요.
      </p>

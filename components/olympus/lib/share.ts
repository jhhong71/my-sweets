...
export const SERVICE_NAME = "나는 어떤 그리스 로마 신을 닮았을까?";

/** 캡처 이미지 배경(투명 영역을 채울 색) — 페이지 배경과 어울리는 톤 */
const CAPTURE_BG = "#F4EFE2";
...
export async function downloadResultImage(node: HTMLElement, id: string): Promise<void> {
  ...
  link.download = `olympus-character-${id}.png`;
  link.href = dataUrl;
  link.click();
}

/** 결과 공유 링크(유형 ID만 포함, 개인 응답은 담지 않는다) */
export function shareUrl(result: TestResult): string {
 ...
 * 실패 안내가 뜨지 않게 한다.
 */
export async function shareResult(result: TestResult): Promise<ShareOutcome> {

...
function reasonText(outcome: ResultOutcome): string {
  ...
  const qual = scores.qual.toFixed(1);
  const trend = scores.trend.toFixed(1);
  const impulse = scores.impulse.toFixed(1);
  return `품질 추구 축 ${qual}점, 브랜드·유행 민감도 축 ${trend}점, 충동 구매 축 ${impulse}점으로 계산되어 이 조합이 '${primary.title}' 기준 프로필과 가장 가까워 이 결과가 나왔어요.`;
}
    ...
    <div className="screen result-screen">
      <article className="result-doc glass-card" ref={captureRef}>
        ...
        <div
          ...
          style={{ background: `linear-gradient(90deg, ${primary.color}, ${primary.colorDeep})` }}
          aria-hidden="true"
        />

        <header className="doc-head">
          <p className="doc-eyebrow">{sharedPreview ? "공유된 결과" : "나의 쇼핑러 유형"}</p>
          <div className="doc-headline">
            <span className="doc-icon">
              <ShopperIcon id={primary.id} size={64} />
            </span>
            <div>
        ...
        <div className="doc-body">
                  ...
                  return (
                    <li key={axis} className="axis-score-row">
                      <span className="axis-score-icon">
                        <ShopperIcon id={AXIS_ICON[axis]} size={34} />
                      </span>
            ...
            <section className="doc-block">
              ...
              <div className="second-row">
                <span className="second-icon">
                  <ShopperIcon id={match.id} size={48} />
                </span>
              ...
              <p className="rank-foot">
                품질 기준과 트렌드 민감도는 나와 비슷하고, 결정 속도만 반대인
                유형이에요. 고르는 취향은 잘 통하면서 한쪽이 지를 때 다른
                한쪽이 속도를 잡아줘서, 같이 쇼핑하면 균형이 맞아요.
              </p>
            </section>
          )}
          ...
          <p className="doc-limit">
            ...
            목적으로 하지 않으며, 문항은 원척도를 번역한 것이 아니라 개념
            정의만 참고해 새로 작성해 원척도와 동일한 타당성이 보장되지는
            않습니다. 상황과 시기에 따라 다른 모습이 나타날 수 있어요.
          </p>

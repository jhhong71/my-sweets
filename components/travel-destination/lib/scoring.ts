...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { pace: 0, scene: 0, plan: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { pace: 0, scene: 0, plan: 0 };
...
export function resultIdFromPoles(poles: Record<Axis, Pole>): ResultId {
  ...
  if (!(key in RESULTS)) {
    throw new Error(`결과 데이터에 없는 여행지 키: ${key}`);
  }
  return key as ResultId;
}

/**
 * 나와 함께 여행하면 잘 맞는 여행 메이트 유형의 축 조합. `plan`(여행 방식)
 * 극만 뒤집고, `pace`(동선 강도)·`scene`(선호 풍경)은 나와 같은 유형을 찾는다.
 *
 * 근거: 여행 방식이 같은 사람끼리는(둘 다 계획형이면 사소한 일정 변경에도
 * 부딪히기 쉽고, 둘 다 즉흥형이면 정작 아무것도 정하지 못한 채 시간을
 * 흘려보내기 쉬운 것처럼) 서로 역할을 보완해주기 어렵다. 여행 방식이 반대면
 * 한쪽이 큰 그림을 짜고 다른 한쪽이 유연하게 채워주는 자연스러운 균형이
 * 생긴다. 반면 동선 강도(pace)와 선호 풍경(scene)까지 다르면 여행하는
 * 속도와 좋아하는 풍경 자체가 어긋나기 쉬워, 이 두 축은 나와 같은 쪽을
 * 찾는다.
 *
 * 이 관계는 대칭이다(A의 궁합 여행지가 B라면 B의 궁합 여행지도 항상 A).
 * 무작위 요소는 없다.
 */
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    plan: poles.plan === "high" ? "low" : "high",
  };
}

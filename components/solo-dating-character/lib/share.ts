...
export const SERVICE_NAME = "나는 어떤 솔로 캐릭터일까?";

/** 캡처 이미지 배경(투명 영역을 채울 색) — 페이지 배경과 어울리는 톤 */
const CAPTURE_BG = "#FDF0F1";
...
export async function downloadResultImage(node: HTMLElement, id: string): Promise<void> {
  ...
  link.download = `solo-character-${id}.png`;
  link.href = dataUrl;
  link.click();
}

/** 결과 공유 링크(캐릭터 ID만 포함, 개인 응답은 담지 않는다) */
export function shareUrl(result: ResultProfile): string {
  const url = new URL(window.location.href);
  url.search = `?character=${result.id}`;
  url.hash = "";
  return url.toString();
}

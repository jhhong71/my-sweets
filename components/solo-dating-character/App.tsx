...
import type { Answers, NameSet, Outcome, ResultId } from "./types";
...
const SOLO_DATING_CHARACTER_TEST_ID = "solo-dating-character";
...
/** 공유 링크(?character=)의 캐릭터 ID. 결과 데이터에 없는 값이면 무시한다. */
function characterIdFromUrl(): ResultId | null {
  const id = new URLSearchParams(window.location.search).get("character");
  return id && id in RESULTS ? (id as ResultId) : null;
}

export default function App() {
  const [screen, setScreen] = useState<Screen>("start");
  const [previousScreen, setPreviousScreen] = useState<Screen>("start");
  const [nameSet, setNameSet] = useState<NameSet>("male");
  const [answers, setAnswers] = useState<Answers>(emptyAnswers);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [outcome, setOutcome] = useState<Outcome | null>(null);
  const [sharedPreviewId, setSharedPreviewId] = useState<ResultId | null>(null);

  /**
   * 공유 링크(?character=...)로 들어온 경우 결과 미리보기로 전환한다.
   * 정적 프리렌더 시점에는 window가 없으므로 마운트 후에 읽는다.
   */
  useEffect(() => {
    const sharedId = characterIdFromUrl();
    if (sharedId) {
      setNameSet(RESULTS[sharedId].set);
  ...
  const start = () => {
    ...
    void recordParticipation(SOLO_DATING_CHARACTER_TEST_ID);
    setScreen("quiz");
  };
  ...
  const selectAnswer = (choiceIndex: number) => {
    ...
    const result = scoreAnswers(nameSet, next);
    if (result) {
      setOutcome(result);
  ...
  const showResult = useCallback(() => {
    setScreen("result");
    if (outcome) history.replaceState(null, "", `?character=${outcome.primary.id}`);
  }, [outcome]);
  ...
  // 직접 테스트한 결과가 있으면 그것을, 없고 공유 링크로 열렸다면 캐릭터 소개를 보여준다.
  const shownResult = outcome
    ? outcome.primary
    : sharedPreviewId
      ? RESULTS[sharedPreviewId]
      : null;

  return (
    <div className="solo-dating-character-app">
      <div className="app-frame">
        ...
        {screen === "result" &&
          (shownResult ? (
            ...
            // 결과도 공유 캐릭터도 없는 상태로 결과 화면에 들어온 경우의 안전한 대체 화면

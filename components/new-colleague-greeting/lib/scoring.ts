...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { approach: 0, curiosity: 0, warmth: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { approach: 0, curiosity: 0, warmth: 0 };
...
/**
 * 나와 궁합이 맞는 유형(=접근 주도권은 서로 보완하면서 알아가는 리듬은 잘 맞는
 * 상대)의 축 조합. `approach`(접근 방식) 극만 뒤집고, `curiosity`(질문 방식)와
 * `warmth`(친밀감 형성 속도)는 나와 같은 유형을 찾는다.
 *
 * 근거: 접근 방식이 같은 사람끼리는(둘 다 적극형이면 누가 먼저 다가갈지
 * 부딪히기 쉽고, 둘 다 지켜보기형이면 아무도 먼저 다가가지 않아 어색한 시간이
 * 길어지기 쉬운 것처럼) 첫 만남의 주도권을 서로 나누기 어렵다. 접근 방식이
 * 반대면 한쪽이 먼저 다가가고 다른 쪽이 자연스럽게 받아주는 균형이 생긴다.
 * 반면 질문 방식(curiosity)과 친밀감 형성 속도(warmth)까지 다르면 알아가는
 * 리듬이 어긋나기 쉬워, 이 두 축은 나와 같은 쪽을 찾는다.
 *
 * 이 관계는 대칭이다(A의 궁합 유형이 B라면 B의 궁합 유형도 항상 A).
 * 무작위 요소는 없다.
 */
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    approach: poles.approach === "high" ? "low" : "high",
  };
}

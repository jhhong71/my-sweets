...
import { AFFILIATE_BY_THEME } from "../../data/affiliate";
...
/** 완성 화면에만 노출되는 제휴 추천 영역. 제휴 URL이 없으면 클릭할 수 없다. */
export function CoupangPartnerRecommendation({ themeId }: Props) {
  const category = AFFILIATE_BY_THEME[themeId];
  const viewed = useRef(false);

  useEffect(() => {
    if (!category || viewed.current) return;
    viewed.current = true;
    trackEvent("coupang_recommendation_view", { themeId });
  }, [category, themeId]);
    ...
    <aside className="coupang-box">
      ...
      {url ? (
        <a
          ...
          onClick={() => trackEvent("coupang_affiliate_click", { themeId })}
        >

...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { closeness: 0, openness: 0, steadiness: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { closeness: 0, openness: 0, steadiness: 0 };
...
/**
 * 나와 연애 궁합이 잘 맞는 유형의 축 조합. `openness`(표현 방식) 극만
 * 뒤집고, `closeness`(밀착도)·`steadiness`(안정도)는 나와 같은 유형을 찾는다.
 *
 * 근거: 감정을 표현하는 방식이 같은 사람끼리는(둘 다 표현형이면 사소한 감정도
 * 서로 부딪히기 쉽고, 둘 다 절제형이면 정작 마음을 확인할 길이 없어 답답해지기
 * 쉬운 것처럼) 서로의 방식을 보완해주기 어렵다. 표현 방식이 반대면 한쪽이
 * 먼저 마음을 꺼내고 다른 한쪽이 편하게 받아주는 자연스러운 균형이 생긴다.
 * 반면 밀착도(closeness)와 안정도(steadiness)까지 다르면 서로 원하는 거리감과
 * 마음의 안정감 자체가 어긋나기 쉬워, 이 두 축은 나와 같은 쪽을 찾는다.
 *
...
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    openness: poles.openness === "high" ? "low" : "high",
  };
}

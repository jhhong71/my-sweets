  ...
  return (
    <main className="screen start-screen">
      <div className="hero">
        ...
        <div className="hero-stage">
          <div className="hero-char">
            <CharacterImage id={RESULT_LIST[0].id} size={190} title="애착 유형 테스트 대표 이미지" />
          </div>
        </div>
      </div>

      <p className="eyebrow">애착 유형 테스트</p>
      ...
      <p className="start-lead">
        연애할 때 나는 어떤 모습일까요?{" "}
        <strong>밀착도·표현도·안정도</strong> 세 가지 축으로, 8가지 유형 중
        나와 가장 가까운 유형을 찾아드려요.
      </p>
      ...
      <ul className="cast" aria-label="만날 수 있는 결과 유형 미리보기">
        {RESULT_LIST.map((result) => (
          <li className="cast-item" key={result.id}>
            <span className="cast-icon">
              <CharacterImage id={result.id} size={56} />
            </span>
      ...
      <p className="disclaimer">
        이 테스트는 선택한 상황과 성향을 바탕으로 결과를 제공하는

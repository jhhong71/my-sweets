...
import type { Answers, Outcome, ResultId } from "./types";
...
const BREATHING_SWORDSMAN_TEST_ID = "breathing-swordsman";
...
/** 공유 링크(?result=)의 캐릭터 ID. 결과 데이터에 없는 값이면 무시한다. */
function resultIdFromUrl(): ResultId | null {
  const id = new URLSearchParams(window.location.search).get("result");
  return id && id in RESULTS ? (id as ResultId) : null;
}

export default function BreathingSwordsmanApp() {
  ...
  const start = () => {
    ...
    void recordParticipation(BREATHING_SWORDSMAN_TEST_ID);
  };
  ...
  return (
    <div className="breathing-swordsman-app">

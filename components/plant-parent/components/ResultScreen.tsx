...
const POLE_PHRASES: Record<string, { high: string; low: string }> = {
  routine: { high: "정해진 루틴대로 계획적으로 관리하는 편", low: "그때그때 상황에 맞춰 즉흥적으로 챙기는 편" },
  care: { high: "작은 변화도 자주, 세심하게 살피는 편", low: "필요할 때가 아니면 여유롭게 두는 편" },
  explore: { high: "새로운 식물이나 방법을 계속 시도하는 편", low: "익숙한 식물과 방식을 오래 유지하는 편" },
};
 ...
 * 하나만 반대로 바뀐 유형이다. (lib/scoring.ts의 secondaryPoles 참고)
 */
  ...
  return (
    <main className="screen result-screen">
      ...
      <div ref={captureRef} className="result-capture">
        <p className="eyebrow">내 반려식물 집사 유형은</p>
        ...
        <p className="capture-note">재미로 보는 반려식물 집사 유형 테스트 · 내 반려식물 집사 유형은?</p>
      </div>
      ...
      <section className="panel">
        <h2 className="panel-title">다음에 식물을 돌볼 때 해보면 좋아요</h2>
      ...
      <p className="limit-note">
        이번 응답에서는 이 유형과 가장 가까웠어요. 식물을 돌보는 방식은
        키우는 식물 종류나 계절, 그때그때 상황에 따라 달라질 수 있고, 이
        결과는 재미로 즐기는 엔터테인먼트예요.
      </p>

...
export function StartScreen({ onStart, onShowPrivacy }: Props) {
  const heroResult = RESULT_LIST[0];
  ...
  return (
    <main className="screen start-screen">
        ...
        <div className="hero-stage">
          <div className="hero-char">
            <CharacterImage
              id={heroResult.id}
              size={130}
              title="반려식물 집사 유형 테스트 대표 이미지"
            />
          </div>
        </div>
      </div>

      <p className="eyebrow">반려식물 집사 유형 테스트</p>
      ...
      <p className="start-lead">
        평소 집이나 사무실에서 식물(화분)을 어떻게 돌보고 있나요?{" "}
        <strong>관리 루틴·관찰 밀도·새로움 추구</strong> 세 가지 축으로,
        8가지 집사 유형 중 나와 가장 가까운 하나를 찾아드려요.
      </p>
      ...
      <ul className="cast" aria-label="만날 수 있는 집사 유형 미리보기">

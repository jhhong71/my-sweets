...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { routine: 0, care: 0, explore: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { routine: 0, care: 0, explore: 0 };
...
/**
 * 두 번째로 가까운 보조 결과의 축 조합. 세 축 중 strength(중간에서 떨어진
 * 정도)가 가장 작은 축 — 이번 응답에서 가장 애매하게 갈린 축 — 하나만 반대로
 * 뒤집고 나머지 두 축은 그대로 유지한다. 그 축만 바뀌면 대표 결과 바로
 * '옆'에 있는 유형이 되므로, 이 유형이 응답상 가장 가까운 이웃이다.
 *
 * 동점(strength가 같은 축이 여러 개) 처리: AXIS_ORDER 순서상 먼저 오는 축을
 * 뒤집는다. 무작위 요소는 없다.
 */
export function secondaryPoles(axes: Record<Axis, AxisResult>): Record<Axis, Pole> {
  let weakest: Axis = AXIS_ORDER[0];
  for (const axis of AXIS_ORDER) {
    if (axes[axis].strength < axes[weakest].strength) weakest = axis;
  }

  const poles = {} as Record<Axis, Pole>;
  for (const axis of AXIS_ORDER) poles[axis] = axes[axis].pole;
  poles[weakest] = poles[weakest] === "high" ? "low" : "high";
  return poles;
}
...
export function resolveOutcome(axes: Record<Axis, AxisResult>): Outcome {
  ...
  return {
    ...
    secondary: RESULTS[resultIdFromPoles(secondaryPoles(axes))],
  };
}

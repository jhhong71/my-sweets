/** 커피 성향 축 3개. 각 축은 서로 반대되는 두 극(pole)을 가진다. */
export type Axis = "intensity" | "sweetness" | "tempo";

/** 축의 두 극. high = 진한형/달콤형/빠른형, low = 부드러운형/담백형/여유형 */
...
/**
 * 8개 결과 유형 키 = 축별 극의 조합
 * (intensity: r/l · sweetness: w/p · tempo: q/s)
 */
...
export type Outcome = {
  ...
  /**
   * 함께 있으면 잘 어울리는 짝 유형. `intensity`(진하기) 극만 반대이고
   * `sweetness`(단맛)·`tempo`(템포)는 나와 같은 유형이다.

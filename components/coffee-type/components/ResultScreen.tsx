...
const POLE_PHRASES: Record<string, { high: string; low: string }> = {
  intensity: { high: "진하고 확실한 자극에 끌리는 편", low: "부드럽고 순한 자극이 편안한 편" },
  sweetness: { high: "달콤한 것으로 기분을 채우는 편", low: "담백하고 꾸밈없는 것이 편안한 편" },
  tempo: { high: "빠르고 신속하게 움직이는 편", low: "여유를 갖고 천천히 움직이는 편" },
};
...
/**
 * 짝 유형은 커피의 진하기만 반대이고 단맛 취향·템포는 나와 같은 유형이다.
 * (src/lib/scoring.ts의 compatiblePoles 참고)
 */
function compatibilityReasonText(outcome: Outcome): string {
  const sweetnessLabel = POLE_LABELS.sweetness[outcome.axes.sweetness.pole];
  const tempoLabel = POLE_LABELS.tempo[outcome.axes.tempo.pole];
  return `커피의 진하기는 나와 반대라서 서로 몰랐던 맛을 소개해주는 재미가 있고, 단맛 취향(${sweetnessLabel})과 움직이는 템포(${tempoLabel})는 나와 같아서 함께 있는 흐름까지 잘 맞아요.`;
}
  ...
  return (
    <main className="screen result-screen">
      ...
      <div ref={captureRef} className="result-capture">
        <p className="eyebrow">나의 커피 유형은</p>

        <section className="result-card" style={{ ["--accent" as string]: result.color }}>
          <div className="result-icon">
            <CoffeeImage id={result.id} size={150} title={`${result.title} 일러스트`} />
          </div>
        ...
        {outcome && (
          <>
            <section className="panel">
              <h2 className="panel-title">이 커피가 나온 이유</h2>
        ...
        <p className="capture-note">재미로 보는 커피 유형 테스트 · 나는 어떤 커피 종류에 가까울까?</p>
      </div>

      {outcome ? (
        <section className="panel secondary-panel">
          <h2 className="panel-title">함께 있으면 잘 어울리는 커피 짝꿍</h2>
          <div className="secondary-row">
            <div
              className="secondary-char"
              style={{ ["--accent" as string]: outcome.compatible.color }}
            >
              <CoffeeImage id={outcome.compatible.id} size={56} />
            </div>
        ...
        <p className="shared-note">
          공유 링크로 열린 <strong>커피 유형 소개</strong> 화면이에요. 내 결과를
      ...
      <section className="panel">
        <h2 className="panel-title">이 커피 유형의 매력</h2>
      ...
      <p className="limit-note">
        이번 응답에서는 이 커피 유형과 가장 가까웠어요. 나의 모습은 그날의
        기분이나 상황에 따라 달라질 수 있고, 이 결과는 재미로 즐기는
        엔터테인먼트예요.
      </p>

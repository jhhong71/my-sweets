  ...
  return (
    <main className="screen start-screen">
      <div className="hero">
        ...
        <div className="hero-stage">
          <div className="hero-char">
            <CoffeeImage id="rps" size={150} title="커피 유형 테스트 대표 이미지" />
          </div>
        </div>
      </div>

      <p className="eyebrow">커피 유형 테스트</p>
      ...
      <p className="start-lead">
        평소의 나를 떠올리며 답하다 보면, 나와 꼭 닮은 커피 한 잔을 찾아드려요.{" "}
        <strong>커피의 진하기·단맛 취향·움직이는 템포</strong> 세 가지 축으로,
        8가지 커피 중 나와 가장 가까운 하나를 알려드려요.
      </p>
      ...
      <ul className="cast" aria-label="만날 수 있는 커피 유형 미리보기">
        {RESULT_LIST.map((result) => (
          <li className="cast-item" key={result.id}>
            <span className="cast-icon">
              <CoffeeImage id={result.id} size={48} />
            </span>
            <span className="cast-name">{result.title}</span>
          </li>
        ))}
      </ul>
      <p className="cast-caption">이런 {RESULT_LIST.length}가지 커피를 만나요</p>
      ...
      <ul className="meta-list" aria-label="테스트 안내">
        ...
        <li>
          <span className="meta-num">{RESULT_LIST.length}</span>가지 커피
        </li>
      </ul>

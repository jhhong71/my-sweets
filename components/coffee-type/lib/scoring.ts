...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { intensity: 0, sweetness: 0, tempo: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { intensity: 0, sweetness: 0, tempo: 0 };
...
/**
 * 함께 있으면 잘 어울리는 짝 유형의 축 조합. `intensity`(진하기) 극만
 * 뒤집고, `sweetness`(단맛)와 `tempo`(템포)는 나와 같은 유형을 찾는다.
 *
 * 근거: 커피 취향에서 진한 커피와 부드러운 커피는 서로 몰랐던 맛을
 * 소개해주며 대화 소재가 되기 좋고(진하기가 다르면 서로의 잔을 나눠
 * 맛보는 재미가 있다), 단맛 취향과 리듬(템포)까지 다르면 함께 있는 흐름
 * 자체가 어긋나기 쉬우므로 이 두 축은 나와 같은 유형을 찾는다.
 *
 * 이 관계는 대칭이다(A의 짝 유형이 B라면 B의 짝 유형도 항상 A).
 * 무작위 요소는 없다.
 */
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    intensity: poles.intensity === "high" ? "low" : "high",
  };
}

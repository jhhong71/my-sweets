...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { decide: 0, company: 0, use: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { decide: 0, company: 0, use: 0 };
...
/**
 * 나와 궁합이 맞는 유형(=결정 방식은 서로 보완하면서 점심시간을 대하는 결은
 * 맞는 상대)의 축 조합. `decide`(결정 방식) 극만 뒤집고, `company`(동행
 * 방식)와 `use`(시간 활용)는 나와 같은 유형을 찾는다.
 *
 * 근거: 결정 방식이 같은 사람끼리는(둘 다 계획형이면 매번 정하는 데 시간이
 * 걸리고, 둘 다 즉흥형이면 정작 아무것도 정하지 못해 우왕좌왕하기 쉬운
 * 것처럼) 서로 결정을 보완해주기 어렵다. 결정 방식이 반대면 한쪽이 정하고
 * 다른 쪽이 따라가는 자연스러운 균형이 생긴다. 반면 함께 있고 싶은
 * 정도(company)와 점심시간을 쓰는 목적(use)까지 다르면 점심시간을 대하는
 * 결이 어긋나기 쉬워, 이 두 축은 나와 같은 쪽을 찾는다.
 *
...
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    decide: poles.decide === "high" ? "low" : "high",
  };
}

...
export const SERVICE_NAME = "업무 스트레스 대처 유형 테스트";

/** 캡처 이미지 배경(투명 영역을 채울 색) — 페이지 배경과 어울리는 톤 */
const CAPTURE_BG = "#FFE9F1";
...
export async function downloadResultImage(node: HTMLElement, id: string): Promise<void> {
  ...
  // 첫 호출에서 일부 요소(캐릭터 이미지 등)가 누락될 수 있어 한 번 예열한 뒤 캡처한다.
  const options = {
  ...
  link.download = `work-stress-coping-${id}.png`;
  link.href = dataUrl;
  link.click();
}

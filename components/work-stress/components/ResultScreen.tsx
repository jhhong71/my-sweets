...
import { downloadResultImage, SERVICE_NAME, shareResult } from "../lib/share";
...
function reasonText(outcome: ResultOutcome): string {
  ...
  const focus = scores.focus.toFixed(1);
  const engage = scores.engage.toFixed(1);
  return `문제중심·감정중심 축 ${focus}점, 접근·거리두기 축 ${engage}점으로 계산된 두 축 조합이 '${primary.title}' 기준 프로필과 가장 가까워 이 결과가 나왔어요.`;
}
...
export function ResultScreen({ outcome, sharedPreview, onRestart, onShowPrivacy }: Props) {
  ...
  return (
    <div className="screen result-screen">
      <article className="result-doc glass-card" ref={captureRef}>
        <GlassTab />
        <Sparkle className="card-sparkle" />
        <div className="doc-accent" style={{ background: `linear-gradient(90deg, ${primary.color}, ${primary.colorDeep})` }} aria-hidden="true" />

        <header className="doc-head">
          <p className="doc-eyebrow">{sharedPreview ? "공유된 결과" : "나의 업무 스트레스 대처 유형"}</p>
          <div className="doc-headline">
            <span className="doc-icon">
              <StressIcon id={primary.id} size={64} />
            </span>
            <div>
        ...
        <div className="doc-body">
          {sharedPreview && (
            <p className="doc-notice" role="note">
              ...
              표시하지 않아요. 나만의 결과를 보려면 테스트를 직접 진행해
              보세요.
            </p>
          )}
                  ...
                  return (
                    <li key={axis} className="axis-score-row">
                      <span className="axis-score-icon">
                        <StressIcon id={AXIS_ICON[axis]} size={34} />
                      </span>
          ...
          {!sharedPreview && (
            <section className="doc-block">
              <h2 className="section-caption">나와 궁합이 잘 맞는 캐릭터</h2>
              <div className="second-row">
                <span className="second-icon">
                  <StressIcon id={secondary.id} size={48} />
                </span>
              ...
              <p className="rank-foot">
                두 축 조합에서 나와 궁합이 잘 맞는 유형은 &lsquo;{secondary.title}&rsquo;예요. 축 점수가 두 번째로 가까워 계산된 궁합이라, 상황과 시기에 따라 이 유형에 가까운 모습이 함께 나타날 수 있어요.
              </p>
            </section>
          )}
          ...
          <p className="doc-limit">
            ...
            인사평가를 목적으로 하지 않으며, 문항은 원 척도를 번역한 것이
            아니라 이론적 구조만 참고해 새로 작성해 원 척도와 동일한 타당성이
            보장되지는 않습니다. 상황과 시기에 따라 다른 모습이 나타날 수
            있어요.
          </p>

          <p className="capture-note">재미로 보는 {SERVICE_NAME}</p>
        </div>
      </article>

...
function reasonText(outcome: ResultOutcome): string {
  ...
  const plan = scores.plan.toFixed(1);
  const save = scores.save.toFixed(1);
  const credit = scores.credit.toFixed(1);
  return `계획적 현금흐름 관리 축 ${plan}점, 저축 우선순위 축 ${save}점, 신중한 카드 사용 축 ${credit}점으로 계산되어 이 조합이 '${primary.title}' 기준 프로필과 가장 가까워 이 결과가 나왔어요.`;
}
    ...
    <div className="screen result-screen">
      <article className="result-doc glass-card" ref={captureRef}>
        ...
        <header className="doc-head">
          <p className="doc-eyebrow">{sharedPreview ? "공유된 결과" : "나의 머니타입"}</p>
          <div className="doc-headline">
            <span className="doc-icon">
              <MoneyIcon id={primary.id} size={64} />
            </span>
            <div>
        ...
        <div className="doc-body">
                  ...
                  return (
                    <li key={axis} className="axis-score-row">
                      <span className="axis-score-icon">
                        <MoneyIcon id={AXIS_ICON[axis]} size={34} />
                      </span>
          ...
          <p className="doc-limit">
            ...
            제작한 엔터테인먼트 콘텐츠입니다. 전문적인 재무 상담이나 심리
            진단을 목적으로 하지 않으며, 문항은 원척도를 번역한 것이 아니라
            개념 정의만 참고해 새로 작성해 원척도와 동일한 타당성이

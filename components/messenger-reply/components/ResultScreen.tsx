...
const POLE_PHRASES: Record<string, { high: string; low: string }> = {
  speed: { high: "바로 답장하는 쪽", low: "여유를 두고 답장하는 쪽" },
  express: { high: "풍성하게 표현하는 쪽", low: "간결하게 답하는 쪽" },
  initiative: { high: "먼저 연락해 이끄는 쪽", low: "오는 연락에 반응하는 쪽" },
};
  ...
  return (
    <main className="screen result-screen">
      ...
      <div ref={captureRef} className="result-capture">
        <p className="eyebrow">나의 답장 스타일은</p>

        <section className="result-card" style={{ ["--accent" as string]: result.color }}>
          <div className="result-icon">
            <ChatIcon id={result.icon} size={112} title={`${result.title} 일러스트`} />
          </div>
        ...
        {outcome && (
          <>
            ...
            <section className="panel">
              <h2 className="panel-title">세 가지 답장 습관 점수</h2>
        ...
        <p className="capture-note">재미로 보는 답장 스타일 테스트 · 나는 어떤 답장 스타일?</p>
      </div>
      ...
      <section className="panel">
        <h2 className="panel-title">다음 대화에서 해보면 좋아요</h2>
      ...
      <p className="limit-note">
        이번 응답에서는 이 유형과 가장 가까웠어요. 답장 습관은 상대와 상황,
        시기에 따라 달라질 수 있고, 이 결과는 재미로 즐기는 엔터테인먼트예요.
      </p>

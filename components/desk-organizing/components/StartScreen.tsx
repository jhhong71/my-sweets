    ...
    <main className="screen start-screen">
      <div className="hero">
        ...
        <div className="hero-stage">
          <div className="hero-char">
            <DeskIcon id="shelf" size={150} title="정리 유형 테스트 대표 이미지" />
          </div>
        </div>
      </div>

      <p className="eyebrow">책상 정리 습관 유형 테스트</p>
      ...
      <div className="bubble start-bubble">
        <p className="start-lead">
          책상과 디지털 파일을 정리할 때 요즘의 나는 어떻게 움직일까요?
          <strong> 정리 방식·보관 성향·정리 리듬</strong> 세 가지로, 8가지
          정리 유형 중 나와 가장 가까운 하나를 찾아드려요.
        </p>
      </div>

      <ul className="axis-preview" aria-label="측정하는 세 가지 정리 습관">
        {AXIS_ORDER.map((axis) => (
      ...
      <ul className="cast" aria-label="만날 수 있는 정리 유형 미리보기">
        {RESULT_LIST.map((result) => (
          <li className="cast-item" key={result.id}>
            <DeskIcon id={result.icon} size={40} />
          </li>
        ))}
      </ul>
      <p className="cast-caption">이런 {RESULT_LIST.length}가지 정리 유형을 만나요</p>

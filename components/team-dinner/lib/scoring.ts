...
export const AXIS_TOTALS: RawScores = (() => {
  const totals: RawScores = { energy: 0, stay: 0, depth: 0 };
...
export function computeAxisResults(answers: Answers): Record<Axis, AxisResult> {
  const raw: RawScores = { energy: 0, stay: 0, depth: 0 };
...
/**
 * 나와 궁합이 맞는 유형(=대화 밀당은 되면서 같은 리듬으로 어울릴 수 있는 상대)의
 * 축 조합. `energy`(사교 에너지) 극만 반대로 뒤집고, `stay`(자리 지속력)와
 * `depth`(대화 온도)는 나와 같은 유형을 찾는다.
 *
 * 근거: 리액터×리액터, 관찰자×관찰자처럼 에너지가 같은 사람끼리는 대화가
 * 한쪽으로만 몰리거나 둘 다 조용해지기 쉽다. 에너지가 반대면 한쪽이 대화를
 * 꺼내고 다른 쪽이 반응하는 자연스러운 밀당이 생긴다. 반면 얼마나 오래
 * 자리를 지키는지(stay)와 어떤 온도의 대화를 원하는지(depth)까지 다르면
 * 같이 있는 시간과 원하는 대화의 결이 어긋나기 쉬워, 이 두 축은 나와 같은
 * 쪽을 찾는다.
 *
...
export function compatiblePoles(poles: Record<Axis, Pole>): Record<Axis, Pole> {
  return {
    ...poles,
    energy: poles.energy === "high" ? "low" : "high",
  };
}

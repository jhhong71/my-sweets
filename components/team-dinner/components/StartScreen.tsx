  ...
  return (
    <main className="screen start-screen">
      <div className="hero">
        ...
        <div className="hero-stage">
          <div className="hero-char">
            <PartyIcon id="mic" size={150} title="회식 자리 유형 테스트 대표 이미지" />
          </div>
        </div>
      </div>

      <p className="eyebrow">회식 자리 유형 테스트</p>
      ...
      <p className="start-lead">
        건배사부터 2차, 마무리까지 회식 자리에서 요즘의 나는 어떻게 움직일까요?
        <strong> 사교 에너지·자리 지속력·대화 온도</strong> 세 가지 축으로, 8가지
      ...
      <ul className="cast" aria-label="만날 수 있는 회식 유형 미리보기">
        {RESULT_LIST.map((result) => (
          <li className="cast-item" key={result.id}>
            <PartyIcon id={result.icon} size={40} />
          </li>
        ))}
      </ul>
